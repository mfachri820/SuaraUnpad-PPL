...
export async function GET(request: Request) {
  try {
    ...
    // TANGKAP ID DAN ROLE DARI HEADERS
    ...
    return successResponse(
      policies,
      "Berhasil mengambil daftar kebijakan",
      200
    );
  } catch (error: unknown) {
    const errorMessage =
...
export async function POST(request: Request) {
  try {
    ...
    if (currentUserRole !== "ADMIN" && currentUserRole !== "LECTURER") {
      return errorResponse(
        "Akses ditolak. Hanya Admin dan Dosen yang dapat membuat kebijakan.",
        403
      );
    }
    ...
    if (!body.title || !body.content) {
      return errorResponse(
        "Data tidak lengkap. title dan content wajib diisi.",
        400
      );
    }
    ...
    return successResponse(
      newPolicy,
      "Wacana kebijakan berhasil dibuat (Status: DRAFT)",
      201
    );
  } catch (error: unknown) {
    const errorMessage =
