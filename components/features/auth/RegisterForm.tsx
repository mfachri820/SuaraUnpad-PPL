    ...
    try {
      ...
      if (result.status === "success") {
        if (result.data?.isNewUser) {
          ...
          toast.success("Akun Google valid! Yuk, lengkapi data akademikmu dulu.");
      ...
      <main className="flex w-full my-10 max-w-sm flex-col p-8 bg-white border border-zinc-100 shadow-xl shadow-zinc-200/50 rounded-3xl">
        ...
        <form onSubmit={handleSubmit(onRegister)} className="w-full space-y-2">
          ...
          <button
            ...
            className="w-full bg-[#ffb656] hover:bg-[#F99D26] text-white font-bold py-4 rounded-xl mt-6 transition-all disabled:opacity-50 hover:cursor-pointer active:scale-95"
          >
        ...
        <div className="mt-8 text-center text-sm text-zinc-500 font-medium w-full">
          Sudah memiliki akun?{" "}
          <a href="/login" className="text-[#2682F9] font-bold hover:underline">
            Masuk di sini
          </a>
        </div>
      </main>
