...
describe('Upload handler and report cleanup', () => {
  ...
  it('calls deleteImageByUrl when report creation fails after an uploaded image URL is provided', async () => {
    ...
    const deleteImageByUrlSpy = vi.spyOn(uploadService, 'deleteImageByUrl').mockResolvedValue(undefined as any);
    (prisma.report.create as any).mockRejectedValueOnce(new Error('Database insertion failed'));

    await expect(reportService.createReport('user-1', payload as any)).rejects.toThrow('Database insertion failed');
