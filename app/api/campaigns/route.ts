...
export async function GET() {
  try {
    ...
    return successResponse(
      campaigns,
      "Berhasil mengambil daftar kampanye",
      200
    );
  } catch (error: unknown) {
    const errorMessage =
  ...
  try {
    ...
    if (userRole !== "ADMIN") {
      return errorResponse(
        "Akses ditolak. Hanya Admin atau Dosen yang dapat membuat kampanye donasi.",
        403
      );
    }
    ...
    if (
      ...
      return errorResponse(
        "Data tidak lengkap. title, description, targetAmount, dan bannerUrl wajib diisi.",
        400
      );
    }
